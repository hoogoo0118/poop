...
function draw() {
    ...
    if (frameCount<360) {
        ...
        text("Pause: ESCAPE; Resume: SPACEBAR",50,480);
        pop();
    }
    ...
    textSize(12);
    fill(255,180);
    text("Framerate: "+frameRate(),260,24);
    textSize(8);
    text("Version 1.0",8,height-10);
    pop();
    ...
    timePlayed=int((millis()-totalTimePaused)/1000);
...
function keyPressed() {
    if (keyCode==32&&isPaused==true) {
        isPaused=false;
        pauseEndTime=millis();
        totalTimePaused+=pauseEndTime-pauseStartTime
        //console.log(totalTimePaused);
        loop();
    }
}
