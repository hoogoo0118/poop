class Player{
    ...
    movement() {
        ...
        if (keyIsDown(RIGHT_ARROW)&&pX<width-8) {
            pX+=pSpeed;
            //print(keyCode);
          } 
    ...
    miss() {
        ...
        for (var i=0; i<enemyEnemy.length; i++) {
            if (dist(pX,pY,enemyEnemy[i].xPos,enemyEnemy[i].yPos)<14) {
                return true;
            }
        }
        return false;
    ...
    graze() {
        for (var i=0; i<enemyShots.length; i++) {
            if (4<dist(pX,pY,enemyShots[i].xPos,enemyShots[i].yPos)&&dist(pX,pY,enemyShots[i].xPos,enemyShots[i].yPos)<30&&enemyShots[i].grazed==false) {
